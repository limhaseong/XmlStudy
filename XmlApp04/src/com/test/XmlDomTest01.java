...
public class XmlDomTest01
{
	public static void main(String[] args)
	{
		...
		try
		{
			...
			// → XML DOM 형성을 위한 준비
			//    (이를 위해 필요한 리소스 구성)
			DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();		//-- newInstance() → static 메소드
																						//-- DocumentBuilderFactory → 추상
			DocumentBuilder builder = factory.newDocumentBuilder();
			Document xmlObj = null;
			
			// XML 파일을 메모리에 코드 → XML DOM 형성
			String url = "VEHICLES.xml";
			xmlObj = builder.parse(url);
			
			// 여기까지 하면 XML DOM 객체를 얻어낸 것.
			...
			for (int i=0; i<vehicleNodeList.getLength(); i++)		//-- 10 번 반복 (i : 0 ~ 9)
			{
				...
				System.out.printf("%s : %s%n", makeElement.getNodeName(), makeElement.getTextContent());
				...
				System.out.printf("%s : %s%n", modelElement.getNodeName(), modelElement.getTextContent());
				*/
